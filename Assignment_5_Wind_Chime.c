...
void timer0Init() {
	...
	PCLKSEL0 &= ~ (1 << 2);				// Reset clock selection on Timer 0, default = 1MHz
	timer0Start();
}
...
void timer2Init() {
	...
	T2EMR |= (1 << 10); // Toggle on match EMR3
	T2EMR |= (1 << 11); // Toggle on match EMR3
	timer2Start();
}
...
void configT2 (int freq) {
	timer0Reset();			// Reset Timer 0
	T2MR3 = timer0Read()/(2*freq);	// load T2MR3 with match value based on frequency PCLK/(2*freq)
	while (timer0Read() < 2000000) {
		int aout = (1 - (timer0Read() / 2000000))*1023;
		DACR = aout << 6;
	}
}
...
int main(void) {
	...
	while(1) {
		configT2(523); // let us try 523 Hz!!
